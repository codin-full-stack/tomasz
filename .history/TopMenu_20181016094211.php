<ul>
    <li><a href="index.php">Home</a></li>
    <li class="log"><a href="login.php">Login</a></li>
    <li><a href="paieska.php">User search</a></li>
    <li><a href="allRegisteredList.php">All users</a></li>
    <li><a href="userInfo.php">My profile</a></li>
    <li><a href="infoChange.php">Profile</a></li>
    <li><a href="post_new.php">New post</a></li>
    <li><a href="myPosts.php">My posts</a></li>
    <li><a href="search.php">Post search</a></li>
    <li class="log"><a href="logout.php">Logout</a></li>
    <li class="kaledos"><a></a></li>
    <li class="clock"><a></a></li>
    <li><input type="text" name="colors" value="" placeholder="body color"></li>
    <li><input type="text" name="navcolor" value="" placeholder="body color"></li>
</ul>


