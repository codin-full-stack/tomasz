...
if (!empty($imagename)) {
    ...
    $title=mysqli_real_escape_string($conn, $_POST['title']);
    $content=mysqli_real_escape_string($conn, $_POST['content']);
    $usql = "UPDATE blogs SET (user_id, title, content, date, image, image_text) VALUES('$pID', '$title', '$content', '$date', '$imagetmp', '$imagename') where post_id = $row['post_id']";
    ...
    header('Location: index.php');
        ...
        $title=mysqli_real_escape_string($conn, $_POST['title']);
        $content=mysqli_real_escape_string($conn, $_POST['content']);
        $usql = "UPDATE blogs SET (user_id, title, content, date) VALUES('$pID', '$title', '$content', '$date') where post_id = $row['post_id']";
        mysqli_query($conn, $usql);
        header('Location: index.php');
    }
